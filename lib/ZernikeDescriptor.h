...
public:
  ...
  ZernikeDescriptor(
      const char
          *_rawName, /**< binary input file name (contains a cubic grid)*/
   ...
   */
  void
  Reconstruct(ComplexT3D &_grid, /**< result grid as 3D complex stl vector */
   ...
   */
  void SaveInvariants(const char *_fName /**< name of the output file */
  );
...
private:
  ...
  void WriteGrid(ComplexT3D &_grid, const char *_fName);

  double ComputeScale_BoundingSphere(T *_voxels, int _dim, T _xCOG, T _yCOG,
                                     T _zCOG);
  double ComputeScale_RadiusVar(T *_voxels, int _dim, T _xCOG, T _yCOG,
                                T _zCOG);
...
private:
  ...
  // T2D                 invariants_;        // 2D vector of invariants under
  // SO(3)
...
// Moved from ZernikeDescriptor.cpp
...
template <class T, class TIn>
ZernikeDescriptor<T, TIn>::ZernikeDescriptor(const char *_rawName, int _order)
    : order_(_order) {
...
template <class T, class TIn> void ZernikeDescriptor<T, TIn>::ComputeMoments() {
 ...
 * the precomputed center of gravity and scaling factor. All the voxels
 * remaining outside the unit ball are set to zero.
 */
template <class T, class TIn> void ZernikeDescriptor<T, TIn>::NormalizeGrid() {
  T point[3];
...
/**
 * Center of gravity and a scaling factor is computed according to the
 * geometrical moments and a bounding sphere around the cog.
 */
template <class T, class TIn>
void ZernikeDescriptor<T, TIn>::ComputeNormalization() {
  ...
  // T recScale = ComputeScale_BoundingSphere (voxels_, dim_, xCOG_, yCOG_,
  // zCOG_);
...
/**
 * Computes the bigest distance from the given COG to any voxel with value
 * bigger than 0.9 I.e. I think a binary volume is implicitly assumed here.
 */
template <class T, class TIn>
double ZernikeDescriptor<T, TIn>::ComputeScale_BoundingSphere(T *_voxels,
                                                              int _dim, T _xCOG,
                                                              T _yCOG,
                                                              T _zCOG) {
  T max = (T)0;
...
/**
 * Computes the average distance from the given COG to all voxels with value
 * bigger than 0.9 I.e. I think a binary volume is implicitly assumed here.
 */
template <class T, class TIn>
double ZernikeDescriptor<T, TIn>::ComputeScale_RadiusVar(T *_voxels, int _dim,
                                                         T _xCOG, T _yCOG,
                                                         T _zCOG) {
  // the edge length of the voxel grid in voxel units
  int d = _dim;
...
template <class T, class TIn>
void ZernikeDescriptor<T, TIn>::Reconstruct(ComplexT3D &_grid, int _minN,
                                            int _maxN, int _minL, int _maxL) {
  ...
  zm_.Reconstruct(_grid,       // result grid
                  ...
                  _minN,
                  _maxN, // min and max freq. components to be reconstructed
                  _minL, _maxL);
}
