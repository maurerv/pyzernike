...
template<typename T>
class PyZernikeDescriptor {
private:
    std::unique_ptr<ZernikeDescriptor<T, T>> descriptor;

public:
    PyZernikeDescriptor(py::array_t<T, py::array::c_style | py::array::forcecast> array, int order) {
    ...
    py::array_t<std::complex<T>> Reconstruct(int dim, int minN = 0, int maxN = 100, int minL = 0, int maxL = 100) {
        using ComplexT = std::complex<T>;
        std::vector<std::vector<std::vector<ComplexT>>> grid(dim, std::vector<std::vector<ComplexT>>(dim, std::vector<ComplexT>(dim)));
